...
def test_a_position_view_survives_any_rearrangement_of_what_it_conceals(
    ...
    assert project_position(position, SEQ, observer) == project_position(
        scrambled(position, observer, data), SEQ, observer
    )
...
def test_an_event_view_survives_any_rearrangement_of_what_it_conceals(commit: Commit, data: st.DataObject) -> None:
    ...
    assert project_transaction(transaction, before, after, observer) == project_transaction(
        transaction,
...
def test_a_position_view_survives_any_substitution_of_what_it_conceals(
    ...
    assert project_position(position, SEQ, observer) == project_position(
        restocked(position, observer, data), SEQ, observer
    )
...
def test_a_position_view_holds_one_entry_for_every_card_on_the_board(
    ...
    view = project_position(position, SEQ, observer)
...
def test_an_event_view_reports_exactly_the_zones_the_observer_reads_differently(
    ...
    event = project_transaction(transaction, before, after, observer)

    seen_before = project_position(before, SEQ, observer).zones
    seen_after = project_position(after, SEQ, observer).zones
