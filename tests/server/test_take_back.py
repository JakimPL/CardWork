...
async def test_the_rules_wait_out_the_window_before_closing_a_round(client: AsyncClient, session: InService) -> None:
...
async def test_a_seat_may_take_its_commitment_back_inside_the_window(client: AsyncClient, session: InService) -> None:
    await close_the_round(client, session)
...
async def test_a_take_back_reopens_the_turn_for_that_seat(client: AsyncClient, session: InService) -> None:
    await close_the_round(client, session)
    await client.post(MOVES, json=reclaiming(1, session.head, "reclaim"), headers=credentials(1))
...
async def test_a_take_back_only_adds_to_the_record(client: AsyncClient, session: InService) -> None:
    await close_the_round(client, session)
    await client.post(MOVES, json=reclaiming(1, session.head, "reclaim"), headers=credentials(1))
...
async def test_a_seat_may_take_back_for_another_seat_at_no_point(client: AsyncClient, session: InService) -> None:
    await close_the_round(client, session)
...
async def test_a_move_opens_the_window_every_seat_takes_its_moment_in(client: AsyncClient, session: InService) -> None:
    await client.post(MOVES, json=sealing(SORTER, DEAL, "sealed"), headers=credentials(SORTER))

    assert session.settling is True


async def test_an_arrangement_opens_no_window_of_its_own(client: AsyncClient, session: InService) -> None:
