...
def test_with_cards_keeps_the_zone_s_identity_and_policy() -> None:
    zone = Zone(
        id="hand:1", owner=1, visibility=HAND, ordered=False, cards=to_game_cards((ACE_OF_SPADES,), face_down=True)
    )
    ...
    assert refilled.ordered is False
    assert refilled.cards == to_game_cards((KING_OF_HEARTS,), face_down=False)


def test_with_cards_keeps_the_arrangement_the_table_holds() -> None:
    zone = Zone(id="draw", visibility=HAND, ordered=True, cards=to_game_cards((ACE_OF_SPADES,), face_down=True))

    refilled = zone.with_cards(to_game_cards((KING_OF_HEARTS,), face_down=False))

    assert refilled.ordered is True
