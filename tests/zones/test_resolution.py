...
from cardwork.zones.resolution import arrangeable_by, audience, visible_to
...
def test_audience_grants_an_ad_hoc_reveal_on_top_of_the_policy() -> None:
    ...
    zone = Zone(
        id="draw",
        visibility=shown_to_one_opponent,
        ordered=True,
    )
...
SPECTATOR_CASES: Final[tuple[SpectatorCase, ...]] = (
    ...
    SpectatorCase(
        ...
        zone=Zone(
            id="draw",
            visibility=Visibility(face_down=Audience.NONE, extra=frozenset({0, 1, 2})),
            ordered=True,
        ),
        card=FACE_DOWN,
        expected=False,
...
ARRANGEMENT_CASES: Final[tuple[ArrangementCase, ...]] = (
    ArrangementCase(
        name="a seat lays out the hand it holds",
        zone=Zone(id="hand:1", owner=1, visibility=HAND, ordered=False),
        observer=1,
        expected=True,
    ),
    ArrangementCase(
        name="a hand is laid out by the seat holding it and by nobody else",
        zone=Zone(id="hand:1", owner=1, visibility=HAND, ordered=False),
        observer=2,
        expected=False,
    ),
    ArrangementCase(
        name="a tray whose run the table keeps stays as it lies for its owner",
        zone=Zone(id="tray:1", owner=1, visibility=HIDDEN, ordered=True),
        observer=1,
        expected=False,
    ),
    ArrangementCase(
        name="a zone the seats share belongs to the table however loosely it lies",
        zone=Zone(id="pool", visibility=PILE, ordered=False),
        observer=1,
        expected=False,
    ),
    ArrangementCase(
        name="a spectator holds no zone to lay out",
        zone=Zone(id="hand:1", owner=1, visibility=HAND, ordered=False),
        observer=None,
        expected=False,
    ),
    ArrangementCase(
        name="a shared loose zone is no spectator's either",
        zone=Zone(id="pool", visibility=PILE, ordered=False),
        observer=None,
        expected=False,
    ),
    ArrangementCase(
        name="a hand its owner cannot read is still its owner's to sort",
        zone=Zone(id="hand:0", owner=0, visibility=HIDDEN, ordered=False),
        observer=0,
        expected=True,
    ),
)
...
@pytest.mark.parametrize("case", ARRANGEMENT_CASES, ids=lambda case: case.name)
def test_arrangeable_by_answers_for_the_seat_asking(case: ArrangementCase) -> None:
    assert arrangeable_by(case.zone, case.observer) is case.expected
