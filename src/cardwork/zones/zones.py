...
def hands(players: int) -> Zones:
    ...
    A hand holds the cards of one seat under an arrangement that seat chooses, since the rules read the cards
    a seat has rather than the run it keeps them in. So a hand is the zone its owner sorts as it pleases.
    """
    return {
        zone_id: Zone(
            id=zone_id,
